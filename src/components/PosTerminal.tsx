    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 relative pb-16 lg:pb-0">
      ...
      <div className={`lg:col-span-7 space-y-4 ${mobileTab === 'products' ? 'block' : 'hidden lg:block'}`}>
                ...
                <>
                  ...
                  <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs">
                    ...
                    <button
                      ...
                      title="When mobile scans, add item to active Cart"
                    >
                      🛒 Add to Cart
                    </button>
                    <button
                      ...
                      title="When mobile scans, deduct stock from garment"
                    >
                      ➖ Deduct (-1)
                    </button>
                  </div>
        ...
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 max-h-[600px] overflow-y-auto pr-1">
            ...
            const itemKey = prod.id ? `pos-prod-${prod.id}-${idx}` : `pos-bar-${prod.barcode || 'item'}-${idx}`;
            return (
              <div
                key={itemKey}
              ...
              >
                ...
                <div className="text-[11px] text-slate-500 mt-1 flex items-center gap-1.5">
                  <span className="font-medium text-slate-700">{((prod.fabricType || 'Pure Cotton').split(' '))[0]}</span>
                  <span>•</span>
                  <span>{prod.size || 'Free Size'}</span>
                </div>
          ...
          <div className="flex-1 overflow-y-auto py-3 space-y-3 min-h-[220px] max-h-[340px]">
            {cart.length === 0 ? (
              ...
              cart.map((item, cartIdx) => {
                const cartKey = item.product?.id
                  ? `pos-cart-${item.product.id}-${cartIdx}`
                  : `pos-cart-bar-${item.product?.barcode || 'item'}-${cartIdx}`;
                return (
                  <div
                    key={cartKey}
      ...
      {completedSale && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setCompletedSale(null);
          }}
        >
          <div
            className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 border border-slate-200 text-center"
